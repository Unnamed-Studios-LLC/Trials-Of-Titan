Shader "Custom/UISprite-AlphaMask"
{
	Properties
	{
		...
		_BoundCount("Bound Count", Int) = 4
		_Bounds("Bounds", Vector) = (1, 0, 0, 0)
		...
		SubShader
		{
			...
			v2f vert(appdata_t v)
			{
				...
				_Bounds = float4(min(OUT.texcoord.y, _Bounds.x), max(OUT.texcoord.y, _Bounds.y), 0, 0);
				_BoundCount += 1;
			...
			*/
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				*/
				...
				//float2 alphaCoord = float2((IN.texcoord.x - bounds.x) / (bounds.z - bounds.x), (IN.texcoord.y - bounds.y) / (bounds.w - bounds.y));
				float mask = 1 - (IN.texcoord.y - _Bounds.x) / ((_Bounds.y - _Bounds.x) / 2); //tex2D(_AlphaMask, alphaCoord).r;
				color.a *= max(mask + 0.4, 0);
